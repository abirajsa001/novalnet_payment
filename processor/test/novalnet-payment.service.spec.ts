...
const novalnetPaymentService =
  ...
  test("getSupportedPaymentComponents", async () => {
    ...
    expect(result?.components[2]?.type).toStrictEqual("Invoice with payment guarantee");
    expect(result?.components[2]?.type).toStrictEqual("Direct debit SEPA with payment guarantee");
    ...
    expect(result?.components[18]?.type).toStrictEqual("WeChat Pay");
